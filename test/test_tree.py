...
class TestSum(unittest.TestCase):
	...
	def test_check_upgma(self):
		...
		self.assertEqual(trees.upgma(df, "out.txt", True),ans)
	...
	def test_check_neighbor_joining(self):
		...
		x = [dog, bear, raccoon, weasel, seal, sea_lion, cat, monkey]
		df = pd.DataFrame(data = x, columns = cols, index = cols)
		ans = '(((dog:12.216666666666669, ((bear:11.666666666666668, raccoon:11.666666666666668):11.966666666666669, (seal:12.0, sea_lion:12.0):11.966666666666669):12.216666666666669):20.383333333333326, weasel:20.383333333333326):42.38333333333334, cat:42.38333333333334), monkey'
		self.assertEqual(trees.neighbor_joining(df, "out.txt", True),ans)
